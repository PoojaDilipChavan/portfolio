...
•	It uses the correct type of visualization i.e. map to represent the state-wise gun death rate.
•	It categorizes data depending on the death rate and uses this information in legends to guide the user. It also mentions the scale of the death rate i.e. per 100k.

Characteristics that did not work well are as follows –
•	Although the US map is familiar to some audiences, it assumes that state names are understood by users and does not mention them on the map.
•	The graph shows data redundancy on the scale for legends with 100k written for each category.
•	The color formatting of the map is not intuitive. It uses dark purple/blue color for fewer deaths which is not intuitive to users. 
•	It does not have a clear title mentioning the purpose of visualization and the year in which data is collected. It also does not have a footnote mentioning the source of the information.

The things I would do differently for this visualization include –
•	Add the state name abbreviations on top of the state map. 
•	I will add an appropriate title like ‘ Gun death rates in 2021 state-wise breakdown’ and also add a footnote with the data source link.
•	I will choose a suitable color theme like shades of red/orange to depict the death rate. 
•	I will also reformat the legends by removing the per 100k notion and adding it on the top of the scale.
